      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mx-auto mt-12 max-w-md space-y-8 sm:mt-16 lg:hidden">
          {tiers.map((tier) => (
            ...
            >
              <h3 id={tier.id} className="text-sm font-semibold text-white">{tier.name}</h3>
              ...
              <a
                ...
                className={classNames(
                  ...
                  'mt-8 block rounded-md px-3 py-2 text-center text-sm font-semibold focus-visible:outline-2 focus-visible:outline-offset-2'
                )}
              >
                Buy plan
              </a>
              <ul role="list" className="mt-10 space-y-4 text-sm text-white">
              ...
              <tbody>
                    ...
                    <td key={tier.id} className="px-6 pt-2 xl:px-8">
                      ...
                      <a
                        ...
                        className={classNames(
                          ...
                          'mt-8 block rounded-md px-3 py-2 text-center text-sm font-semibold text-white focus-visible:outline-2 focus-visible:outline-offset-2'
                        )}
                      >
                        Buy plan
                      </a>
                ...
                {sections.map((section, sectionIdx) => (
                  <Fragment key={section.name}>
                    ...
                    {section.features.map((feature) => (
                      <tr key={feature.name}>
                        <th scope="row" className="py-4 text-sm font-normal text-white">
                          {feature.name}
                        ...
                        {tiers.map((tier) => {
                          ...
                          return (
                            <td key={tier.id} className="px-6 py-4 xl:px-8">
                              {typeof value === 'string' ? (
                                <div className="text-center text-sm text-gray-300">{value}</div>
                              ) : (
                                <>
                                  {value === true ? (
